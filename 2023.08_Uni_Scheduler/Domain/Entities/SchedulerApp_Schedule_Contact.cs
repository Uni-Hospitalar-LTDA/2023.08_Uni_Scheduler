...
{
    public class SchedulerApp_Schedule_Contact : Querys<SchedulerApp_Schedule_Contact>
    {
        ...
        public async static Task deleteAsync(string idSchedule)
        {
            using (SqlConnection conn = new Connection().getConnectionApp())
            {
                try
                {
                    ...
                    command.CommandText = $"DELETE FROM [UHCDB].dbo.[SchedulerApp_Schedule_Contact] WHERE idSchedule = {idSchedule}";
